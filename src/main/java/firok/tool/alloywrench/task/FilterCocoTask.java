	...
	)
	{
		...
		try
		{
			...
			for(var annotationOld : annotationsOld)
			{
				...
				// info 全是黑名单策略
				if(setImageIdFilter.contains(imageId)) continue;
				if(setCategoryIdFilter.contains(categoryId)) continue;
				if(setAnnotationIdFilter.contains(annotationId)) continue;
