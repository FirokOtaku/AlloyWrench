...
{
	public static int PIECE_X = 800;
	public static int PIECE_Y = 800;
	public static int OVERLYING_X = 150;
	public static int OVERLYING_Y = 150;

	/**
	 * 裁剪出标签的最小面积
	 * */
	public static int LIMIT_AREA_ANNOTATION = 16;

	/**
	 * 裁剪出标签图形点之间的最小距离
	 * */
	public static int LIMIT_DISTANCE_COOR = 2;

	public static boolean CHECK_FS = false;
	public static boolean SHOULD_READ_IMAGE = true;
	public static boolean SHOULD_WRITE_SUB_IMAGE = true;
...
//				throw new IllegalArgumentException("COCO 数据文件未声明任何种类");
		...
		var smt1 = new SimpleMultiThread(
				...
				() -> { // 加载图片数据
					...
					images.stream().parallel().forEach(img -> {
						...
						var fileImage = "".equals(pathImageBase) ? new File(filename) : new File(folderImage, filename);
		...
		images.stream().parallel().forEach(img -> {
			...
			for(var range : ranges) // 遍历裁剪范围 里面就不需要再套一层多线程了
			{
				...
				var filenameSub = "%s%s(%d,%d-%d,%d).png".formatted(
						filenamePrefix, filenameShort,
